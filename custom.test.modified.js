...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      await transaction({
        ...
        recipient: recipient,
        isL1Withdrawal: false, // change this to false
      })

      const aliceBalance = await token.balanceOf(tornadoPool.address) // this is the remaining balance on L1
      //console.log(aliceBalance)
      expect(aliceBalance).to.be.equal(utils.parseEther('0.02')) // difference between deposit and withdrawal, 0.1 - 0.08 = 0.02
      const recipientBalance = await token.balanceOf(recipient) // this is the balance on L2
      //console.log(recipientBalance)
      expect(recipientBalance).to.be.equal(aliceWithdrawAmount)
      const omniBridgeBalance = await token.balanceOf(omniBridge.address) // this is the balance on the bridge
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      const aliceEthAddress = '0x000000000000000000000000000000000000dEaD' // https://etherscan.io/address/0x000000000000000000000000000000000000dead
      ...
      const aliceWithdrawAmount = utils.parseEther('0.07') // 0.13 - 0.06 = 0.07
      ...
      await transaction({
        ...
        isL1Withdrawal: true, // change this to false
      })
